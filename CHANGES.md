...
- Request: continue improving the platform autonomously; make the crowding desk trustworthy.
- Attempted: ported the positioning-stress scoring to an offline reconstruction (`src/lib/crowdingHistory.ts`), event-studied 120 days of public candles + funding (25 assets, 72k funding rows) with pre-registered production thresholds, and verified exact parity between the TS port and the original Python study.
- Decision: ship the pooled backtest stats as a static JSON (`src/data/crowding-track-record.json`, regenerated via `npm run research:crowding-track` with paced/backoff HL fetches) instead of a runtime endpoint — zero serverless cost, no DB, and the numbers are always labeled "backtest, not live results".
- Result: fading high-severity price-confirmed alerts scored +1.11% net over 24h (n=84, 5/5 months positive) and +3.77% over 72h (n=64) vs +0.36% unconditional drift; flat-to-negative inside 8h, which validates the "watch, don't chase" copy. UI shows a track-record footnote in the crowding desk panel and a "what usually followed" note on high/extreme per-asset stress cards. Tests (`test:crowding-history`, 6/6), lint, tsc, build, and an SSR smoke on `next start` all passed.

## 2026-07-14

- Request: "I go on the website but am confused on what to trade. I should never be confused."
- Attempted: audited the decision surfaces (DailySetupPanel picks one setup and buries it in a sidebar; crowding, radar, and movers each speak separately), extracted `buildDailySetupCandidates` so all funding setups are available, moved the crowding payload builder into `src/lib/crowdingDesk.ts` shared by both routes, and composed everything in `src/lib/tradeIdeas.ts`.
- Decision: one ranked answer at the top of /markets — at most three ideas, conviction A/B/C (A = stressed positioning with price confirmation, the backtested condition), each with trigger/stop/target, horizon, one-line thesis, "wrong if", and the track-record note on A-tier crowding ideas; explicit stand-aside when nothing qualifies. HIP-3 builder markets are excluded: view-only in the app, thin, and outside the backtest basket.
- Result: `/api/ideas` (60s per-network cache) + `TradeIdeasPanel` full-width above the market grid. 8 unit tests on composition rules (merge same-coin agreement to A, crowding wins side conflicts, medium severity excluded per backtest, HIP-3 excluded, capped at 3, stand-aside). tsc, lint, build clean; live smoke: daily-setup and crowding routes unregressed, panel SSRs, ideas endpoint returned one honest B-tier watch (CASHCAT fade) with main-dex conditions unconfirmed.
