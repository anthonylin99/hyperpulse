...
export function MarketProvider({ children }: { children: ReactNode }) {
  ...
  const fetchSignalData = useCallback(async (assets: MarketAsset[]) => {
    const now =
      Math.floor(Date.now() / SIGNAL_CANDLE_BUCKET_MS) *
      SIGNAL_CANDLE_BUCKET_MS;
    ...
    for (const asset of topAssets) {
      try {
        ...
        const fundingHistory = (
          Array.isArray(fundingRaw) ? fundingRaw : []
        ).map((f: { time: number; fundingRate: string }) => ({
  ...
  const fetchMarketData = useCallback(async () => {
    try {
      ...
      const top10 = [...parsed]
        ...
        .slice(0, 10);
      if (
        shouldFetchMarketEnrichment &&
        now - fundingFetchRef.current >= MARKET_ENRICHMENT_INTERVAL_MS
      ) {
      ...
      if (
        shouldFetchMarketEnrichment &&
        now - btcCandleFetchRef.current >= MARKET_ENRICHMENT_INTERVAL_MS
      ) {
