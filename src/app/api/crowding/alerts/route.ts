...
async function fetchAssets(requestUrl: URL): Promise<MarketAsset[]> {
  const info = getInfoClient(resolveNetworkFromRequest(requestUrl));
  const [main, ...hip3] = await Promise.all([
    info.metaAndAssetCtxs(),
    ...HIP3_DEXS.map((dex) =>
      info.metaAndAssetCtxs({ dex }).catch((error) => {
        logServerError(`api/crowding/alerts:dex:${dex}`, error);
        return null;
      }),
    ),
  ]);
...
async function fetchFundingHistory(asset: MarketAsset, requestUrl: URL): Promise<FundingPoint[]> {
  const info = getInfoClient(resolveNetworkFromRequest(requestUrl));
...
async function buildPayload(requestUrl: URL): Promise<CrowdingDeskPayload> {
  const now = Date.now();
  const assets = (await fetchAssets(requestUrl))
  ...
  const fundingEntries = await Promise.allSettled(
    assets.map(async (asset) => [asset.coin, await fetchFundingHistory(asset, requestUrl)] as const),
  );
...
export async function GET(request: Request) {
  ...
  const network = resolveNetworkFromRequest(requestUrl);
  ...
  try {
    ...
    if (!inFlight) {
      inFlight = buildPayload(requestUrl).finally(() => {
